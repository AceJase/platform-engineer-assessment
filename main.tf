...
resource "azurerm_key_vault" "functions_kv" {
  ...
  # Left with all perms as assumed to be a superuser requiring full access to
  # the kv
  access_policy {
    tenant_id = data.azurerm_client_config.current.tenant_id
...
module "function_app" {
  # This would be a versioned GitHub release in practice.
  source         = "git@github.com:AceJase/tfmodule-function_app.git?ref=main"
